...
struct operation_gt {
    ...
    inline operation_gt(op_code_t op_type, std::size_t op_count) : type(op_type), count(op_count) {}
};
...
void serializability_mix() {
    ...
    auto task_insert = [&]() {
        ...
        for (std::size_t txn_idx = 0; txn_idx != txn_cnt_ak; ++txn_idx) {
            ...
            operation_t operation(operation_t::insert_k, op_cnt);
            random_fill(operation.keys, op_cnt);
            random_fill(operation.values, op_cnt);

            transaction_t txn = db.transact().throw_or_release();
            auto txn_ref = txn[strided_range_gt<ukv_key_t>(operation.keys.data(), op_cnt)];
    ...
    auto task_remove = [&]() {
        for (std::size_t txn_idx = 0; txn_idx != txn_cnt_ak; ++txn_idx) {
            std::size_t op_cnt = dist(gen);
            operation_t operation(operation_t::remove_k, op_cnt);
            random_fill(operation.keys, op_cnt);
    ...
    auto task_select = [&]() {
        for (std::size_t txn_idx = 0; txn_idx != txn_cnt_ak; ++txn_idx) {
            std::size_t op_cnt = dist(gen);
            operation_t operation(operation_t::select_k, op_cnt);
            random_fill(operation.keys, op_cnt);
