...
typedef enum {
    doc_nothing_k = -1,
    doc_modify_upsert_k = 0,
    doc_modify_update_k = 1,
    doc_modify_insert_k = 2,
    doc_modify_patch_k = 3,
    doc_modify_merge_k = 4,
    doc_modify_remove_k = 5,
} doc_modification_t;
...
void modify_field( //
    ...
    if (yyjson_mut_is_arr(val)) {
        ...
        if (c_modification == doc_modify_merge_k) {
        ...
        else if (c_modification == doc_modify_insert_k) {
            if (is_idx) {
...
void patch( //
    ...
    while ((obj = yyjson_mut_arr_iter_next(&arr_iter))) {
        ...
        if (yyjson_mut_equals_str(op, "add")) {
            ...
            nested_path ? modify_field(original_doc, value, nested_path, doc_modify_insert_k, c_error)
            ...
            auto nested_path = field_concat(field, yyjson_mut_get_str(path), arena, c_error);
            return_on_error(c_error);
            nested_path ? modify_field(original_doc, nullptr, nested_path, doc_modify_remove_k, c_error)
            ...
            auto nested_path = field_concat(field, yyjson_mut_get_str(path), arena, c_error);
            return_on_error(c_error);
            nested_path ? modify_field(original_doc, value, nested_path, doc_modify_update_k, c_error)
                        : yyjson_mut_doc_set_root(original_doc, value);
        }
            ...
            auto nested_path = field_concat(field, yyjson_mut_get_str(path), arena, c_error);
            return_on_error(c_error);
            modify_field(original_doc, value, nested_path, doc_modify_upsert_k, c_error);
        }
            ...
            return_on_error(c_error);
            modify_field(original_doc, nullptr, nested_from_path, doc_modify_remove_k, c_error);
            auto nested_to_path = field_concat(field, yyjson_mut_get_str(path), arena, c_error);
            return_on_error(c_error);
            modify_field(original_doc, value, nested_to_path, doc_modify_upsert_k, c_error);
        }
    }
}
...
void modify( //
    ...
    if (field && c_modification != doc_modify_patch_k) {
    ...
    else if (c_modification == doc_modify_patch_k)
...
void read_modify_unique_docs( //
    ...
    if (c_modification == doc_nothing_k)
    ...
    bool need_values = has_fields || c_modification == doc_modify_patch_k || c_modification == doc_modify_merge_k;
    ...
    if (need_values) {
        ...
        for (std::size_t task_idx = 0; task_idx != places.size(); ++task_idx, ++found_binary_it) {
            ...
            return_if_error(has_fields || c_modification != doc_modify_insert_k, c_error, 0, "Key Already Exists!");
            ukv_str_view_t field = places.fields_begin ? places.fields_begin[task_idx] : nullptr;
            value_view_t binary_doc = *found_binary_it;
            json_t parsed = any_parse(binary_doc, internal_format_k, arena, c_error);
        ...
        for (std::size_t task_idx = 0; task_idx != places.size(); ++task_idx) {
            ...
            return_if_error(!presents[task_idx] || c_modification != doc_modify_insert_k,
                            c_error,
                            0,
                            "Invalid Arguments!");
            callback(task_idx, field, parsed);
