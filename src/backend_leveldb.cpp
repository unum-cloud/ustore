...
void measure_many( //
    ...
    std::fill_n(lens.begin(), tasks.count, 0);
...
void read_many( //
    ...
    span_gt<byte_t> tape = arena.alloc<byte_t>(lens_bytes, c_error);
    if (*c_error)
        return;
    ...
    *c_found_values = reinterpret_cast<ukv_val_ptr_t>(tape.begin() + lens_bytes);
}
