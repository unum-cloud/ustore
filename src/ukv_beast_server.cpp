 ...
 * > POST /txn/client:  Returns: {id?: int, error?: str}
 * > DELETE /txn/id:    Drops the transaction and it's contents.
 * > PATCH /txn/id:     Commits and drops the transaction.
 *
...
template <typename body_at, typename allocator_at, typename send_at>
...
class session_t : public std::enable_shared_from_this<session_t> {
    ...
    struct send_request_t {
        ...
        explicit send_request_t(session_t& self) : self_(self) {}

        template <bool isRequest, class body_at, class Fields>
        void operator()(http::message<isRequest, body_at, Fields>&& msg) const {
            // The lifetime of the message has to extend
            // for the duration of the async operation so
            // we use a shared_ptr to manage it.
            auto sp = std::make_shared<http::message<isRequest, body_at, Fields>>(std::move(msg));
    ...
    send_request_t lambda_;

  public:
    // Take ownership of the stream
    session_t(tcp::socket&& socket, std::shared_ptr<std::string const> const& doc_root)
  ...
  private:
    ...
    void on_accept(beast::error_code ec, tcp::socket socket) {
        if (ec) {
            ...
            std::make_shared<session_t>(std::move(socket), doc_root_)->run();
        }
...
int main(int argc, char* argv[]) {
    ...
    if (argc != 5) {
        std::cerr << "Usage: ukv_beast_server <address> <port> <doc_root> <threads>\n"
                  << "Example:\n"
