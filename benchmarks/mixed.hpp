...
namespace unum::ustore::bench {

using doc_w_key_t = std::pair<ustore_key_t, value_view_t>;
...
void docs_upsert( //
    ...
    docs_write.modification = ustore_doc_modify_upsert_k;
    docs_write.arena = arena.member_ptr();
    docs_write.type = ustore_doc_field_json_k;
    docs_write.tasks_count = batch_size;
    docs_write.collections = &collection;
    docs_write.keys = batch_keys.data();
    docs_write.keys_stride = sizeof(ustore_key_t);
    ...
    ustore_transaction_t transaction = nullptr;
    ...
    ustore_transaction_commit_t transaction_commit {};
    ...
    for (auto _ : state) {
        ...
        ustore_transaction_commit(&transaction_commit);
        if (status) {
...
void paths_upsert( //
    ...
    paths_write.paths = (ustore_str_view_t*)batch_paths.front().member_ptr();
    ...
    paths_write.values_bytes = (ustore_bytes_cptr_t*)batch_values.front().member_ptr();
