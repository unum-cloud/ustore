...
class keys_stream_t;
class keys_vals_stream_t;
class keys_range_t;
class keys_vals_range_t;
struct size_range_t;
...
class keys_vals_stream_t {
  ...
  public:
    ...
    keys_vals_stream_t& operator=(keys_vals_stream_t&&) = default;

    keys_vals_stream_t(keys_vals_stream_t const&) = delete;
    keys_vals_stream_t& operator=(keys_vals_stream_t const&) = delete;
     ...
     */
    keys_vals_stream_t& operator++() noexcept {
...
class keys_vals_range_t {
  ...
  public:
    ...
    keys_vals_range_t(keys_vals_range_t const&) = default;
    keys_vals_range_t& operator=(keys_vals_range_t const&) = default;

    expected_gt<keys_vals_stream_t> find_begin(
        std::size_t read_ahead = keys_vals_stream_t::default_read_ahead_k) noexcept {
        keys_vals_stream_t stream {db_, col_, read_ahead, txn_};
        status_t status = stream.seek(min_key_);
        return {std::move(status), std::move(stream)};
    }

    expected_gt<keys_vals_stream_t> find_end() noexcept {
    ...
    keys_vals_stream_t begin(std::size_t read_ahead = keys_stream_t::default_read_ahead_k) noexcept(false) {
        auto maybe = find_begin(read_ahead);
        maybe.throw_unhandled();
        return *std::move(maybe);
    }

    keys_vals_stream_t end() noexcept(false) {
        auto maybe = find_end();
        maybe.throw_unhandled();
        return *std::move(maybe);
    }
