...
#include "ustore/cpp/ranges_args.hpp"
...
template <>
struct location_store_gt<ustore_key_t> : public location_store_gt<collection_key_field_t> {
    using location_store_gt<collection_key_field_t>::location_store_gt;
};
template <>
...
template <>
struct locations_in_collection_gt<ustore_key_t> : public collection_key_field_t {};
...
struct places_arg_extractor_gt {
    ...
    ustore_size_t count(location_t const& arg) noexcept {
    ...
    strided_iterator_gt<ustore_key_t const> keys(location_t const& arg) noexcept {
        ...
        if constexpr (std::is_same_v<element_t, ustore_key_t>)
            return strided;
        else
            return strided.members(&element_t::key);
    }

    strided_iterator_gt<ustore_collection_t const> collections(location_t const& arg) noexcept {
    ...
    strided_iterator_gt<ustore_str_view_t const> fields(location_t const& arg) noexcept {
...
struct places_arg_extractor_gt<places_arg_t> {
    ...
    ustore_size_t count(places_arg_t const& native) noexcept { return native.count; }
    strided_iterator_gt<ustore_key_t const> keys(places_arg_t const& native) noexcept { return native.keys_begin; }
    strided_iterator_gt<ustore_collection_t const> collections(places_arg_t const& native) noexcept {
        return native.collections_begin;
    }
    strided_iterator_gt<ustore_str_view_t const> fields(places_arg_t const& native) noexcept {
...
struct places_arg_extractor_gt<locations_in_collection_gt<at>> {
    ...
    ustore_size_t count(location_t const& arg) noexcept { return base_t {}.count(arg.without.ref()); }
    strided_iterator_gt<ustore_key_t const> keys(location_t const& arg) noexcept {
...
struct contents_arg_extractor_gt<char const**> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(char const** strings) noexcept {
        return {(ustore_bytes_cptr_t const*)strings, sizeof(char const*)};
    }
    strided_iterator_gt<ustore_length_t const> offsets(char const**) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> lengths(char const**) noexcept { return {}; }
};

template <>
struct contents_arg_extractor_gt<char const*> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(char const*& one) noexcept {
        return {(ustore_bytes_cptr_t const*)&one, 0};
    }
    strided_iterator_gt<ustore_length_t const> offsets(char const*) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> lengths(char const*) noexcept { return {}; }
};
...
struct contents_arg_extractor_gt<char const [length_ak]> {
    ...
    strided_iterator_gt<ustore_length_t const> offsets(char const*) noexcept {
        return {};
    }
    strided_iterator_gt<ustore_length_t const> lengths(char const*) noexcept {
        return {};
    }
};
...
struct contents_arg_extractor_gt<value_view_t[length_ak]> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(value_view_t* many) noexcept {
        return {many->member_ptr(), sizeof(value_view_t)};
    }
    strided_iterator_gt<ustore_length_t const> offsets(value_view_t*) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> lengths(value_view_t* many) noexcept {
...
struct contents_arg_extractor_gt<value_view_t> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(value_view_t& one) noexcept { return {one.member_ptr(), 0}; }
    strided_iterator_gt<ustore_length_t const> offsets(value_view_t) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> lengths(value_view_t& one) noexcept { return {one.member_length(), 0}; }
};

template <>
struct contents_arg_extractor_gt<contents_arg_t> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(contents_arg_t native) noexcept {
        return native.contents_begin;
    }
    strided_iterator_gt<ustore_length_t const> offsets(contents_arg_t native) noexcept { return native.offsets_begin; }
    strided_iterator_gt<ustore_length_t const> lengths(contents_arg_t native) noexcept { return native.lengths_begin; }
};

template <>
struct contents_arg_extractor_gt<std::nullptr_t> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(std::nullptr_t) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> offsets(std::nullptr_t) noexcept { return {}; }
    strided_iterator_gt<ustore_length_t const> lengths(std::nullptr_t) noexcept { return {}; }
};

template <typename allocator_at>
struct contents_arg_extractor_gt<std::vector<value_view_t, allocator_at>> {
    strided_iterator_gt<ustore_bytes_cptr_t const> contents(std::vector<value_view_t, allocator_at> const& many) noexcept {
        return many.empty()
                   ? strided_iterator_gt<ustore_bytes_cptr_t const> {}
                   : strided_iterator_gt<ustore_bytes_cptr_t const> {many.front().member_ptr(), sizeof(value_view_t)};
    }
    strided_iterator_gt<ustore_length_t const> offsets(std::vector<value_view_t, allocator_at> const&) noexcept {
        return {};
    }
    strided_iterator_gt<ustore_length_t const> lengths(std::vector<value_view_t, allocator_at> const& many) noexcept {
        return many.empty()
                   ? strided_iterator_gt<ustore_length_t const> {}
                   : strided_iterator_gt<ustore_length_t const> {many.front().member_length(), sizeof(value_view_t)};
    }
};
